    ...
    document.addEventListener("DOMContentLoaded", () => {
        // URL of your API endpoints
        const entrepreneurshipsUrl = "https://apiemprendelink-production-c376.up.railway.app/api/entrepreneurships";
        const entrepreneursUrl = "https://apiemprendelink-production-c376.up.railway.app/api/entrepreneurs"; // Add entrepreneurs API endpoint
        ...
        const fetchData = async () => {
            try {
                // Fetch entrepreneurships
                const entrepreneurshipsResponse = await fetch(entrepreneurshipsUrl);
                if (!entrepreneurshipsResponse.ok) {
                    throw new Error("Error fetching entrepreneurships data");
                }
                const entrepreneurshipsData = await entrepreneurshipsResponse.json();
                const entrepreneurship = entrepreneurshipsData[0];

                // Fetch entrepreneurs
                const entrepreneursResponse = await fetch(entrepreneursUrl);
                if (!entrepreneursResponse.ok) {
                    throw new Error("Error fetching entrepreneurs data");
                }
                const entrepreneursData = await entrepreneursResponse.json();
                const entrepreneur = entrepreneursData[0];

                // Update Hero Section
                if (entrepreneurship.name) {
                    heroSection.querySelector("h1").textContent = entrepreneurship.name;
                }
                if (entrepreneurship.logo_path) {
                    heroSection.querySelector(".hero-logo").src = entrepreneurship.logo_path;
                    heroSection.querySelector(".hero-logo").alt = `${entrepreneurship.name} Logo`;
                }

                // Update Profile Section
                if (entrepreneur) {
                    profileSection.innerHTML = `
                        <img src="${entrepreneur.image}" alt="${entrepreneur.name} ${entrepreneur.lastname}" class="profile-image">
                        <div class="profile-info">
                            <h2>${entrepreneur.name} ${entrepreneur.lastname}</h2>
                            <p><i class="fas fa-envelope"></i> <a href="mailto:${entrepreneur.email}">${entrepreneur.email}</a></p>
                            <p><i class="fas fa-map-marker-alt"></i> ${entrepreneur.location}</p>
                            <p><i class="fas fa-phone"></i> ${entrepreneur.phone}</p>
                            <p><i class="fas fa-birthday-cake"></i> ${new Date(entrepreneur.birth_date).toLocaleDateString()}</p>
                        </div>
                    `;
                }

                // Update Description Section
                if (entrepreneurship.general_description) {
                    descriptionSection.querySelector("h3").textContent = "Descripción del Emprendimiento";
                    descriptionSection.innerHTML = `
                        <h3>Descripción del Emprendimiento</h3>
                        <p>${entrepreneurship.general_description}</p>
                        <div>
                            <strong>Categoría:</strong> ${entrepreneurship.category || 'No especificada'}<br>
                            <strong>Especificaciones:</strong> ${entrepreneurship.specifications || 'No disponible'}
                        </div>
                    `;
                }
                ...
                if (entrepreneurship.product_images && entrepreneurship.product_descriptions) {
                    const productImages = JSON.parse(entrepreneurship.product_images);
                    const productDescriptions = JSON.parse(entrepreneurship.product_descriptions);
                        ...
                        const productElement = `
                            <div class="product">
                                ...
                                <div class="product-info">
                                    ...
                                    <p>${productDescriptions[index] || 'Sin descripción'}</p>
                                </div>
                            </div>
                        `;
                ...
                profileSection.innerHTML = "<p>Error al cargar los datos del emprendedor. Intente nuevamente más tarde.</p>";
                descriptionSection.innerHTML = "<p>Error al cargar los datos del emprendimiento. Intente nuevamente más tarde.</p>";
                productsContainer.innerHTML = "<p>Error al cargar los productos. Intente nuevamente más tarde.</p>";
            }
        };
